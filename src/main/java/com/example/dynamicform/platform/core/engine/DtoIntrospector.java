...
public class DtoIntrospector {
    ...
    public RawFormMetadata buildMetadata(List<FieldSpecRequest.FieldSpec> fieldSpecs, String rootModelName, Class<?> targetClass, Set<String> enabledReferenceModels, Map<String, Object> requestAttributes) {
        ...
        String topLevelName = segments[0];
            ...
            try {
                ...
                if (segments.length == 1) {
                    topLevelFields.putIfAbsent(topLevelName, buildDynamicAttribute(topLevelName, spec, normalizedValidations));
                }
            }
        }
    ...
    private void buildNestedFields(RawDomainAttribute parent,
        ...
        if (isLast && child != null && !Boolean.TRUE.equals(child.getReference())) {
            child.setVisible(leafSpec.getVisible() != null ? leafSpec.getVisible() : child.getVisible());
            child.setShortLabel(leafSpec.getShortLabel() != null ? leafSpec.getShortLabel() : child.getShortLabel());
            child.setShortLabelI18n(leafSpec.getShortLabelI18n() != null ? leafSpec.getShortLabelI18n() : child.getShortLabelI18n());
            child.setLongLabel(leafSpec.getLongLabel() != null ? leafSpec.getLongLabel() : child.getLongLabel());
            child.setLongLabelI18n(leafSpec.getLongLabelI18n() != null ? leafSpec.getLongLabelI18n() : child.getLongLabelI18n());
        }
    ...
    private RawDomainAttribute buildDynamicAttribute(String attributeName,
                                                     FieldSpecRequest.FieldSpec spec,
                                                     List<Map<String, Object>> normalizedValidations) {
        RawDomainAttribute attribute = RawDomainAttribute.builder()
                .attributeName(attributeName)
                .attributeType(String.class.getSimpleName())
                .modelName(String.class.getSimpleName())
                .referenceModel(attributeName)
                .reference(false)
                .collection(false)
                .visible(spec.getVisible() != null ? spec.getVisible() : true)
                .shortLabel(spec.getShortLabel() != null ? spec.getShortLabel() : "")
                .shortLabelI18n(spec.getShortLabelI18n())
                .longLabel(spec.getLongLabel() != null ? spec.getLongLabel() : "")
                .longLabelI18n(spec.getLongLabelI18n())
                .build();
        if (!normalizedValidations.isEmpty()) {
            attribute.setValidations(normalizedValidations);
        }
        return attribute;
    }
