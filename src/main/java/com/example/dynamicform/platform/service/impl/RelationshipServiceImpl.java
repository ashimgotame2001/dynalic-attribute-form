...
import com.example.dynamicform.platform.repository.RelationshipDefinitionRepository;
...
@Service("platformRelationshipService")
@Transactional
public class RelationshipServiceImpl implements RelationshipService {
    ...
    public void deleteRelationshipDefinition(String relationshipName) {
        definitionRepository.findByRelationshipName(relationshipName).ifPresent(definitionRepository::delete);
    }

    @Override
    @Transactional(readOnly = true)
    public Optional<RelationshipDefinitionDTO> getRelationshipDefinition(String relationshipName) {
        return definitionRepository.findByRelationshipName(relationshipName).map(this::toDto);
    }

    @Override
    @Transactional(readOnly = true)
    public List<RelationshipDefinitionDTO> getAllRelationshipDefinitions() {
        return definitionRepository.findAll().stream().map(this::toDto).collect(Collectors.toList());
    }

    @Override
    @Transactional(readOnly = true)
    public List<RelationshipDefinitionDTO> getRelationshipDefinitionsByEntity(String entityName) {
        return definitionRepository.findByEntityInvolved(entityName).stream().map(this::toDto).collect(Collectors.toList());
    }
    ...
    private void validateDefinition(RelationshipDefinitionDTO definition) {
        if (definition == null) throw new IllegalArgumentException("Relationship definition is required");
        if (isBlank(definition.getSourceEntity())) throw new IllegalArgumentException("sourceEntity is required");
        if (isBlank(definition.getTargetEntity())) throw new IllegalArgumentException("targetEntity is required");
        if (definition.getRelationshipType() == null) throw new IllegalArgumentException("relationshipType is required");
    }
    ...
    private RelationshipDefinitionEntity toEntity(RelationshipDefinitionDTO dto, RelationshipDefinitionEntity entity) {
        ...
        entity.setDescription(defaultIfBlank(dto.getDescription(), entity.getSourceEntity() + " -> " + entity.getTargetEntity()));
    ...
    private String normalizeRelationshipName(RelationshipDefinitionDTO definition) {
        if (!isBlank(definition.getRelationshipName())) return definition.getRelationshipName().trim();
